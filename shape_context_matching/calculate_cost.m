function ssd_global=calculate_cost(V1,V2)
...
nbins_theta=12;   %angular bins
nbins_r=5;        %radial bins
r_inner=1/8;      %innner radius i.e. radius of first radial bin
r_outer=2;        %outer radius i.e. radius of outermost radial bin
tan_eps=1.0;
...
[N1,N2]=size(V1);     %Dimensions of V1 
...
% get boundary points
%disp('extracting boundary points...Stay Calm')
[x1,y1,t1]=bdry_extract_3(V1);

nsamp1=length(x1);      %no. of sample points of image V1
if nsamp1>=nsamp
...
%disp('Going in loop to compute distances')
while s
   %disp(['iter=' int2str(k)])
...
%   cvec=hungarian_fast(costmat2);
   ...
   %printf("The value of sc_cost");
   
   
   % warp each coordinate( overlapping the two shapes)
